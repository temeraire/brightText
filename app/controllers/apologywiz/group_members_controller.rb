...
class Apologywiz::GroupMembersController < ApologywizController
  ...
  def create
    ...
    if @user.present?
      SharingMailer.invitation_registered(@owner, @user, "ApologyWiz").deliver
    else
      SharingMailer.invitation(@owner.email, @group_member.email, "ApologyWiz").deliver
    end
  ...
  def update
    ...
    respond_to do |format|
      if @group_member.update_attributes(params[:story_set_category])
        format.html { redirect_to('/apologywiz/story_set_categories?filter=' + @group_member.application_id.to_s) }
        format.xml  { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      ...
      format.html { redirect_to apologywiz_story_set_categories_path(:filter => @group_member.application_id.to_s) }
      format.xml  { head :ok }
    end
  end
end
