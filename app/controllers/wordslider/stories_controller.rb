class Wordslider::StoriesController < WordsliderController
  ...
  def new
    ...
    respond_to do |format|
      format.html { render :layout => 'wordslider' }
      format.xml  { render :xml => @story }
    end
  end
...
#        format.html { render :action => "new" }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to("/wordslider/stories?filter=" + @story.story_set_id.to_s) }
  ...
  def reorder_stories_rank
    if( params[:story_set_id].blank? )
      redirect_to wordslider_stories_path
  ...
  def update_stories_rank
    ...
    redirect_to wordslider_stories_path(:filter => params[:filter])
  end
