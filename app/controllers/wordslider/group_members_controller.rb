...
class Wordslider::GroupMembersController < WordsliderController
  ...
  def update
    ...
    respond_to do |format|
      if @group_member.update_attributes(params[:story_set_category])
        format.html { redirect_to('/wordslider/story_set_categories?filter=' + @group_member.application_id.to_s) }
        format.xml  { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      ...
      format.html { redirect_to wordslider_story_set_categories_path(:filter => @group_member.application_id.to_s) }
      format.xml  { head :ok }
    end
  end
end
