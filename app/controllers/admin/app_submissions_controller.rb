class Admin::AppSubmissionsController < ApplicationController
  ...
  def update
    ...
    respond_to do |format|
      if @app_submission.update_attributes(params[:app_submission])
        format.html { redirect_to(admin_app_submission_path(@app_submission), :notice => 'App submission was successfully updated.') }
        format.xml  { head :ok }
      else
