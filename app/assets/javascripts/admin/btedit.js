var BrightTextEditor = function(divId, editable) {
    ...
    this.renderContainer = function(div, data, pos, randomize, preserveChoice) {
        ...
        if (data.length == 0)
            return;  // ignore line-break containers
        var container = $("<p/>");
        ...
        for (var i in pile.elements) {
            ...
            if (typeof(pileElement.choiceSetIds) != 'undefined' && pileElement.choiceSetIds.length > 0 && self._toneFilter) {
                ...
                for (var j = 0; j < pileElement.choiceSetIds.length; j++) {
                    ...
                    if (choice == self._toneFilter)
                        add = true;
                }
            }
            if (add)
                pileEls.push(pileElement);
        }
    ...
    this._cpUpdateSelection = function(pile, elementId, el) {
        ...
        el.innerHTML = value;
        if (typeof(element.choiceSetIds) !=="undefined" && element.choiceSetIds.length > 0) {
            ...
            $.each(element.choiceSetIds, function() {
                if (this == self._toneFilter)
                    toneChangeRequired = false;
            });
...
(function($) {
    ...
    function showmenu(event, choices, filter) {
        ...
        $.each(tonedChoices, function() {
            self._drawChoice(this, true);
        });
        $.each(untonedChoices, function() {
            self._drawChoice(this, false);
        });
    ...
    self._drawChoice = function(choice, toned) {
        ...
        if (!toned)
            actionElement.addClass("untoned");
        ...
        if (actionElement.width() > 300)
            actionElement.width(300);
...
(function($) {
    ...
    function resetMenu(event) {
        ...
        while (tg != null) {
            if (container == tg || (submenu && submenu[0] == tg))
                return;
            tg = tg.parentNode;
        }
        $(container).hide().empty();
...
(function($) {
    ...
    function resetMenu(event) {
        var tg = event ? event.target : event;
        while (tg != null) {
            if (container == tg || originalTarget == tg)
                return;
            tg = tg.parentNode;
        }
        $(container).hide().empty();
